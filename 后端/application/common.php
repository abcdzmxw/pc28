 ...
 * 中英数字混合的字符串截取
 * Param: 参数说明
 * 【in】$sourcestr string  截取字符串
 * 【in】$cutlength int  截取长度
 */
function assoc_substr($sourcestr, $cutlength): string
{
    $returnstr = '';
    $i = 0;
    $n = 0;
...
if (!function_exists('redis')) {
    ...
    {
        $redis = new Redis();
        $isTrue = $redis->connect(config('redis.host'),config('redis.port'),);
        if (!$isTrue){
            return false;
        }
...
if (!function_exists('randCnStr')){
    function randCnStr(int $num,int $type = 0): string
    {
        $b = '';
        for ($i=0;$i<$num;$i++){
            switch ($type){
                case 0:
//                    $a = chr(mt_rand(0xA0,0xFE)).chr(mt_rand(0xA0,0xFE));
//                    try {
//                        $b .= iconv('Big5', 'UTF-8', $a);
//                    }catch (Throwable $e){
//                        continue 2;
//                    }
                    $a = chr(mt_rand(0xA0,0xFE)).chr(mt_rand(0x40, 0x7E));
                    $b .= iconv('Big5', 'UTF-8//IGNORE', $a);
                    break;
                case 1:
