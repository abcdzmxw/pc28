...
class Auth
{
     ...
     * @return boolean
     */
    public function register($username, $password, $email = '', $mobile = '', $extend = []): bool
    {
        // 检测用户名、昵称、邮箱、手机号是否存在
...
//        }
        ...
        $sign_gift = (int)Db::name('config')->where('id',20)->value('value');
        $vm_gift = (int)Db::name('config')->where('id',21)->value('value');
//        $sign_gift = $vm_gift;
        if ($extend['group_id'] == 1){
            $params['money'] = $sign_gift;
        }elseif ($extend['group_id'] == 2){
            $params['score'] = $vm_gift;
        }
    ...
    public function getUserinfo(): array
    {
        ...
        $nowDay = Carbon::createFromTime(0,0,0);
        $nowDayl = Carbon::createFromTime(0,0,0)->addDay();
        $groupData = Db::name('quantity_log')
            ->whereBetween( 'createtime',[$nowDay->getTimestamp(),$nowDayl->getTimestamp()])
            ->where(['user_id'=>$this->id,])
            ->where('type','in',['2','3'])
            ->group('type')
            ->field(['type','sum(quantity) as sums'])
//            ->buildSql(true);
            ->select();
         $type = array_column($groupData,'type');
         $groupData = array_combine($type,$groupData);
         if (empty($groupData["3"])) $groupData['3']['sums'] = '0';
         if (empty($groupData["2"])) $groupData['2']['sums'] = '0';
         $userinfo['profit'] = bcadd($groupData['3']['sums'],$groupData['2']['sums'],0);
         $userinfo['profit'] = bcadd($groupData['3']['sums'],$groupData['2']['sums'],0);
        $nowDay = Carbon::createFromTime(0,0,0)->subDay();
        $nowDayl = Carbon::createFromTime(0,0,0);
        $groupData = Db::name('quantity_log')
            ->whereBetween( 'createtime',[$nowDay->getTimestamp(),$nowDayl->getTimestamp()])
            ->where(['user_id'=>$this->id,])
            ->where('type','in',['2','3'])
